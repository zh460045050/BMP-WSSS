...
def background_aware_propagtation(ref_mod=None, images=None, labels=None, cams=None, segs=None, img_box=None):
    
    cams = torch.cat([segs[:, 0, :, :].unsqueeze(1).clone(), cams], dim=1)
...
def multi_scale_cam_with_aff_mat_seg(model, inputs, scales, propagation="BSP"):
    cam_list, aff_mat, seg_list = [], [], []
    b, c, h, w = inputs.shape
    with torch.no_grad():
        inputs_cat = torch.cat([inputs, inputs.flip(-1)], dim=0)

        #_cam, _aff_mat = model(inputs_cat, cam_only=True)
        _, _seg, _aff_mat, _cam = model(inputs_cat,)
        ...
        _seg = (_seg[:b,...] + _seg[b:,...].flip(-1)) / 2
        seg_list.append(_seg)
        #msc_segs = _seg
        ####
        ...
        ####
        msc_segs = torch.sum(torch.stack(seg_list, dim=0), dim=0)
        msc_segs = msc_segs + F.adaptive_max_pool2d(-msc_segs, (1, 1))
        msc_segs /= F.adaptive_max_pool2d(msc_segs, (1, 1)) + 1e-5
        ####

        #msc_segs = torch.max(torch.stack(seg_list, dim=0), dim=0)[0]#.unsqueeze(0)

    if propagation == "AFA":
        max_aff_mat = aff_mat[np.argmax(scales)]
        return cam, max_aff_mat, msc_segs
    else:
        return cam, cam, msc_segs
...
def refine_cam_with_bkg_v3_test(ref_mod=None, images=None, labels=None, cams=None, img_box=None):
...
def refine_cams_with_par_test(ref_mod=None, images=None, cams=None, cls_labels=None, cfg=None, img_box=None, down_scale=2):

    _images = F.interpolate(images, size=[cams.shape[2], cams.shape[3]], mode="bilinear", align_corners=False)
    b,_,h,w = _images.shape
    ...
    cams_with_bkg_l = torch.cat((bkg_l, cams), dim=1)
    _cams_with_bkg_l = F.interpolate(cams_with_bkg_l, size=[cams.shape[2], cams.shape[3]], mode="bilinear", align_corners=False)#.softmax(dim=1)
...
def refine_cams_with_bkg_v3(ref_mod=None, images=None, cams=None, cls_labels=None, cfg=None, img_box=None, down_scale=2):
    ...
    return refined_label, refined_label_h, refined_label_l
